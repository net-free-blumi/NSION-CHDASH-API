...
class ProductManager {
    ...
    createProductCard(code, product) {
        ...
        // אם אין שם למוצר אבל יש שם לחיפוש, הצג את שם החיפוש בכותרת
        if (!productName && product.searchName) {
            productInfo = productInfo.replace(
                '<h3></h3>',
                '<h3 style="color: #6c757d; font-style: italic;">שם לחיפוש: ' + product.searchName + '</h3>'
            );
        }
