...
app.post('/send-whatsapp', async (req, res) => {
    try {
        ...
        const url = `${GREEN_BASE_URL}/sendMessage/${GREEN_API_TOKEN}`;
        const greenBody = {
            chatId: groupId,
            message: message
        };
        ...
        if (!response.ok) {
            ...
            console.error('Green API error:', response.status, errText);
            return res.status(500).json({ error: 'שגיאה בשליחת ההודעה ל-Green API' });
        }

        const data = await response.json().catch(() => ({}));
        return res.json({ success: true, green: data });
    } catch (error) {
        console.error('send-whatsapp error:', error);
        return res.status(500).json({ error: 'שגיאה בשרת בעת שליחת הודעת WhatsApp' });
    }
});
